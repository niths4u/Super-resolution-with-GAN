...
import torchvision.models as models  ##NN
import scipy 
import scipy.misc
...
class GeneratorLoss(nn.Module):
    def __init__(self):
        ...
        model_type = "vgg"
        if model_type == "vgg":
            model_load = vgg16(pretrained=True)
        elif model_type == "convnext":
            model_load = models.convnext_base(pretrained=True)
        
        #convnext_base = models.convnext_base(pretrained=True) ##NN
        loss_network = nn.Sequential(*list(model_load.features)[:31]).eval()
        for param in loss_network.parameters():
           param.requires_grad = False
        self.loss_network = loss_network
        #self.loss_network = nn.MSELoss()  ###NN
